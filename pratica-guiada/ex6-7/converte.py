...
é a validação de entrada (formato numérico, valores não finitos como
NaN/infinito, unidade e limite físico do zero absoluto), não o cálculo
em si, que é trivial.
"""
...
def validar_valor(valor_texto: str) -> float:
    ...
    Também rejeita explicitamente NaN e infinito ("nan", "inf", "-inf",
    "Infinity", ...): tecnicamente `float()` aceita esses textos, mas eles
    não representam uma temperatura válida. Sem essa checagem extra, um
    valor "nan" passaria em silêncio pela validação do zero absoluto
    (qualquer comparação com NaN é falsa) e o programa imprimiria "nan"
    ...
    try:
        ...
        return  # erro() sempre encerra o programa; nunca chega aqui.

    if math.isnan(valor) or math.isinf(valor):
        erro(
            f"'{valor_texto}' não é uma temperatura numérica válida. "
            "Valores como NaN, inf ou -inf não são aceitos."
        )
        return  # erro() sempre encerra o programa; nunca chega aqui.
